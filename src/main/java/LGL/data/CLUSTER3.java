...
public class CLUSTER3 implements Comparable {
    ...
    public CLUSTER3(PET pet, int extensionLength) {
    ...
    public CLUSTER3(PET pet, int extensionLength, int sortingLabel, Hashtable<String, Integer> chromLength) {
        head = null;
        tail = null;
        this.weight = 1.0;
        this.setAnnotation("---");
    ...
    public CLUSTER3(REGION head, REGION tail) {
        this.head = head;
        this.tail = tail;
        this.weight = 1.0;
        this.setAnnotation("---");
    ...
    public CLUSTER3(REGION head, REGION tail, double weight) {
        this.head = head;
        this.tail = tail;
        this.weight = weight;
        this.setAnnotation("---");
    ...
    public CLUSTER3 (REGION head, REGION tail, double weight, int sortingLabel) {
        this.head = head;
        this.tail = tail;
        this.weight = weight;
        this.setAnnotation("---");
    ...
    public CLUSTER3(REGION head, REGION tail, double weight, String annotation) {
        this.head = head;
        this.tail = tail;
        this.weight = weight;
    ...
    public static Vector<REGION3> cluster2Region3(CLUSTER3 cluster) {
    ...
    public static Vector<REGION3> cluster2Region3(Vector<CLUSTER3> clusters) {
        Vector<REGION3> region3s = new Vector<REGION3>();
    ...
    public static Vector<CLUSTER3> load(String ClusterFile) throws IOException {
        Vector<CLUSTER3> clusters = new Vector<CLUSTER3>();
        ...
        while ((line = fileIn.readLine()) != null) {
            ...
            CLUSTER3 cluster = new CLUSTER3(head, tail, weight);
    ...
    public static Vector<CLUSTER3> load_orderSpecific(String ClusterFile) throws IOException {
        ...
        while ((line = fileIn.readLine()) != null) {
            ...
            CLUSTER3 cluster = new CLUSTER3(head, tail, weight, sortingLabel);
            cluster.setPetIndexes(petIndexes);
            clusters.add(cluster);
        }
        fileIn.close();
    ...
    public static int getMaxRegionSpan(Vector<CLUSTER3> clusters) {
        int maxRegionSpan = 0;
    ...
    public static void saveClusters(String ClusterFile, Vector<CLUSTER3> clusters) throws IOException {
    ...
    public static Hashtable<String, Vector<CLUSTER3>> cluster2Hash(Vector<CLUSTER3> clusters) {
        // separate the clusters by head chromosomes
        Hashtable<String, Vector<CLUSTER3>> cluster2Hash = new Hashtable<String, Vector<CLUSTER3>>();
        for (int i = 0; i < clusters.size(); i++) {
            Vector<CLUSTER3> clustersTemp = cluster2Hash.get(clusters.elementAt(i).getHead().getChrom());
            if (clustersTemp == null) {
                clustersTemp = new Vector<CLUSTER3>();
        ...
        if (chromNames != null) {
            ...
            while (itr.hasNext()) {
                ...
                Vector<CLUSTER3> clustersTemp = (Vector<CLUSTER3>) cluster2Hash.get(chromName);
    ...
    public static boolean clusterOverlap(CLUSTER3 cluster, Hashtable<String, Vector<CLUSTER3>> cluster2Hash, int maxRegionSpan) {
        ...
        if (clustersTemp != null) {
            ...
            if (cluster.overlap(((Vector<CLUSTER3>) clustersTemp).elementAt(index)) == true) {
                overlapped = true;
                return overlapped;
            }
            ...
            for (int iCluster = index - 1; iCluster >= 0; iCluster--) {
                ...
                if (cluster.overlap(((Vector<CLUSTER3>) clustersTemp).elementAt(iCluster)) == true) {
                    overlapped = true;
                    return overlapped;
                } else if (cluster.getHead().getStart() - ((Vector<CLUSTER3>) clustersTemp).elementAt(iCluster).getHead().getStart() > maxRegionSpan) {
                    break;
                }
//                } else if (cluster.getHead().overlap(((Vector<CLUSTER3>) clustersTemp).elementAt(iCluster).getHead()) == false) {
//                    break;
//                }
            }
    ...
    public static void clusterOverlap(CLUSTER3 cluster, Hashtable<String, Vector<CLUSTER3>> cluster2Hash, int maxRegionSpan, PrintWriter fileOut) throws IOException {
        ...
        if (clustersTemp != null) {
            ...
            //System.out.println(cluster.toString() + " <==> " + ((Vector<CLUSTER3>) clustersTemp).elementAt(index).toString()) ;
            ...
            if (cluster.overlap(((Vector<CLUSTER3>) clustersTemp).elementAt(index)) == true) {
                ...
                fileOut.print("\t" + ((Vector<CLUSTER3>) clustersTemp).elementAt(index).toString());
            }
            ...
            for (int iCluster = index - 1; iCluster >= 0; iCluster--) {
                ...
                if (cluster.overlap(((Vector<CLUSTER3>) clustersTemp).elementAt(iCluster)) == true) {
                    ...
                    fileOut.print("\t" + ((Vector<CLUSTER3>) clustersTemp).elementAt(iCluster).toString());
                } else if (cluster.getHead().getStart() - ((Vector<CLUSTER3>) clustersTemp).elementAt(iCluster).getHead().getStart() > maxRegionSpan) {
                    break;
                }
            }
    ...
    public void combine(CLUSTER3 anotherCluster) throws Exception {
    ...
    public int compareTo(Object anotherCluster) throws ClassCastException {
        if (!(anotherCluster instanceof CLUSTER3)) {
            throw new ClassCastException("A CLUSTER object expected.");
        }
        int result = this.getHead().getChrom().compareTo(((CLUSTER3) anotherCluster).getHead().getChrom());
        if (result == 0) {
            result = this.getTail().getChrom().compareTo(((CLUSTER3) anotherCluster).getTail().getChrom());
        }
        /*if (result == 0) {
    ...
    public boolean overlap(CLUSTER3 anotherCluster) {
        boolean overlapped = false;
    ...
    public boolean overlap_orderSpecific(CLUSTER3 anotherCluster) {
        boolean overlapped = false;
    ...
    public int calculateClusterAnchorBoundaryDistance(CLUSTER3 anotherCluster) {
        if (this.overlap(anotherCluster)) {
            return 0; // if the clusters are overlapped at both anchors, the CLUSTER3 anchor boundary distance is 0
        }
