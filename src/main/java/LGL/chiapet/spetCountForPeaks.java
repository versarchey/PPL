 ...
 * @author ligl
 */
public class spetCountForPeaks {
    ...
    public spetCountForPeaks(String peakFile, String petFile, String outputFile) throws IOException {
        rightNow = Calendar.getInstance();
        System.out.println("[" + rightNow.getTime().toString() + "] start spetCountForPeaks ... ");
        // read clusters
    ...
    public static void main(String[] args) throws IOException {
        if (args.length == 3) {
            new spetCountForPeaks(args[0], args[1], args[2]);
        } else {
            System.out.println("Usage: java spetCountForPeaks <peak_file> <pet_file> <output_file>");
            System.exit(1);
        }
    }
    ...
    void updatePeaks(String petFile, HashMap<String, ArrayList<RegionConcise>> peakHash) throws IOException {
        ...
        while ((line = fileIn.readLine()) != null) {
            ...
            if (fields[0].compareToIgnoreCase(fields[3]) == 0) { // intra-chromosomal
                updatePeaks(fields[0], Integer.parseInt(fields[1]), Integer.parseInt(fields[4]), peakHash, 0);
            }
        }
        fileIn.close();
    }

    void updatePeaks(String chrom, int start, int end, HashMap<String, ArrayList<RegionConcise>> peakHash, int mode) {
