...
print("Ploting the distribution of the length of fragments...")
    ...
    while line is not None and line != "":
        try:
            ...
            if(line.split("\t")[10]=="passed"):
                data_r.append(int(line.split("\t")[5])-int(line.split("\t")[4])+1)
...
# plt.yscale('log')  # 设置x轴为对数刻度
# plt.xticks(np.arange(0, data.max(), 1000))
plt.xticks(xticks)
plt.xlim(0, )
plt.xlabel("Fragment Length")
plt.title("Distribution of fragments length (density)")
...
# sns.ecdfplot(data=df, x='data', hue='hue')
plt.xlim(0, )
plt.xlabel("Fragment Length")
plt.xticks(xticks)
plt.title("Distribution of fragments length (stacking density)")
