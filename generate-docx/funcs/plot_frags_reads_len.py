...
with open(contactsFileName,"r") as dataReader:
    ...
    while line is not None and line != "":
        ...
        try:
            # if(line.split("\t")[5]!="-1"):
            if(line.split("\t")[10]=="passed"):
                data_f.append(int(line.split("\t")[5])-int(line.split("\t")[4])+1)
                hue.append("Real fragment")
        except:
            print(line)
        line = dataReader.readline()
data_r=list(data_r.values())
hue=hue+["Sequencing read"]*len(data_r)
...
plt.title("Distribution of reads length (density)")
...
plt.xlabel("Read Length")
plt.xticks(xticks[:])
plt.title("Distribution of reads length (stacking density)")
# plt.grid(True, linestyle='--', alpha=1)
...
text3=plt.text(mid_f,0.51,va="bottom",s=mid_f)

text4=plt.text(1,0.26,va="bottom",s="lower")
text5=plt.text(q1_r,0.26,va="bottom",s=q1_r)
# text6=plt.text(q1_v,0.26,va="bottom",s=q1_v)
text7=plt.text(q1_f,0.26,va="bottom",s=q1_f)

text8=plt.text(1,0.76,va="bottom",s="higher")
text9=plt.text(q3_r,0.76,va="bottom",s=q3_r)
# text10=plt.text(q3_v,0.76,va="bottom",s=q3_v)
text11=plt.text(q3_f,0.76,va="bottom",s=q3_f)
# texts=[text0,text1,text2,text3,text4,text5,text6,text7,text8,text9,text10,text11]
texts=[text0,text1,text3,text4,text5,text7,text8,text9,text11]
